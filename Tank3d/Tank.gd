...
onready var bullet_point = $Turret/Gun/Direction
...
func _input(event):
	if event is InputEventMouseMotion:
		turret.rotate_y(deg2rad(-event.relative.x * mouse_sensivity))
		
		var x_delta = event.relative.y * mouse_sensivity
		if gun_x_rotation + x_delta > -6 and gun_x_rotation + x_delta < 8:
			gun.rotate_x(deg2rad(-x_delta))
			gun_x_rotation += x_delta
...
func _process(delta):
	...
	if Input.is_action_pressed("fire") and can_fire:
		...
		bullet_instance.rotation_degrees = turret.rotation_degrees
